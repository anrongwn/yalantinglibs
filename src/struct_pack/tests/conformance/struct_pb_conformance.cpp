...
class harness {
  std::pair<std::errc, ConformanceResponse> run_test(const ConformanceRequest& request) {
    std::cout << request.message_type << std::endl;
    auto descriptor = DescriptorPool::generated_pool()->FindMessageTypeByName(request.message_type);
//    if (descriptor == nullptr) {
//      std::cout << "No such message type: " << request.message_type << std::endl;
//      return {
//        std::errc::no_message, {}
//      };
//    }
